...
{
    //Mise à jour de m_model hérité de la classe Renderable
...
glm::mat4 HierarchicalRenderable::computeTotalGlobalTransform() const
{
    if( m_parent )
    {
        //Si l'objet a un parent, on retourne sa trasformation locale multipliée par la transformation globale de son parent
        return m_parent -> computeTotalGlobalTransform() * m_globalTransform ;
    }
    else
    {
        //Si l'objet n'a pas de parents, on retourne simplement sa transformation globale
        return m_globalTransform;
    }
    return glm::mat4();
}
