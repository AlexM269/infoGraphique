...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
    ...
    //spot interne
    float cos_point_to_light = dot(surfel_to_light, -light.spotDirection);
    float intensity = 0.0;
    if(cos_point_to_light > light.innerCutOff){
         intensity = 1.0;
    }
    //spot externe, transition d'intensité lumineuse entre 1 et 0 selon éloignement du point pour effet réaliste
    if(cos_point_to_light < light.innerCutOff && cos_point_to_light > light.outerCutOff){
        intensity = clamp((cos_point_to_light-light.outerCutOff)/(light.innerCutOff-light.outerCutOff),0.0,1.0);
    }
